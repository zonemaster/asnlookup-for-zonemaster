...
while (<>) {
	...
	die qq(ERROR on line $linecnt in input data: "$prefix" is no a valid IPv6 subnet mask >= 0 and <= 64)
	    unless $prefix =~ /^\d+$/ and $prefix >= 0 and $prefix <= 64;
	...
	die qq(ERROR on line $linecnt in input data: "$prefix" is no a valid IPv4 subnet mask >= 0 and <= 24)
	    unless $prefix =~ /^\d+$/ and $prefix >= 0 and $prefix <= 24;
